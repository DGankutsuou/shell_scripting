...
vcpu=$(grep -c "^processor" /proc/cpuinfo) #Provides each processor with an identifying number.
...
fram=$(free -m | grep "Mem:" | awk '{print $2}') #Display the free/unused memory

uram=$(free -m | grep "Mem:" | awk '{print $3}') #Diplay the used memory

pram=$(free | grep "Mem:" | awk '{printf("%.2f", $3/$2*100)}') #Display the usage rate as a percentage
...
fdisk=$(df -BG | grep '^/dev/' | grep -v '/boot$' | awk '{printf ("%d\n", $2)}')
